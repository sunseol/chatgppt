...
describe("live golden path image model evidence", () => {
  test("blocks image artifacts that omit model or prompt version evidence", () => {
    // Given
    const imageArtifacts = [
      imageArtifact(1, { modelOrRuntime: "" }),
      imageArtifact(2, { promptVersion: " " }),
      imageArtifact(3),
      imageArtifact(4),
      imageArtifact(5),
    ];
    ...
    // When
    const result = evaluateLiveGoldenPathE2EBundle(completeBundle({ imageArtifacts }));
...
function completeBundle(patch: {
  ...
  return {
    ...
    sources,
    lineage: [...goldenPathTextLineage(), ...patch.imageArtifacts],
...
function imageArtifact(
  ...
  return createProviderArtifactProvenance({
    ...
    modelOrRuntime: overrides.modelOrRuntime ?? "gpt-image-1",
    promptVersion: overrides.promptVersion ?? "slide_background@v1",
    durationMs: 1_000,
