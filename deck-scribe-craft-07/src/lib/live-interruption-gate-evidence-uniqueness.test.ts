...
describe("live interruption gate evidence uniqueness", () => {
  test("blocks interrupted artifact gates that reuse one approval and export evidence path", () => {
    // Given
    const matrix = completeMatrix({
      scenarios: LIVE_INTERRUPTION_SCENARIOS.map((id) =>
        id === "interrupted_artifact_gate"
          ? scenario(id, {
              approvalGateEvidencePath: "recovery/interrupted-artifact-gate.json",
              exportGateEvidencePath: "recovery/interrupted-artifact-gate.json",
            })
          : scenario(id),
      ),
    });
