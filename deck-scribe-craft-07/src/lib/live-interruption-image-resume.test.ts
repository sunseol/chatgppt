...
describe("live interruption image partial resume evidence", () => {
  test("blocks partial resume claims that resume an image outside the pending set", () => {
      ...
      scenarios: LIVE_INTERRUPTION_SCENARIOS.map((id) =>
        id === "image_partial_resume"
          ? scenario(id, {
              ...
              resumedArtifactIds: ["img_003", "img_004", "img_untracked"],
            })
          : scenario(id),
      ),
    });
    ...
    expect(result.issues[0]?.refs).toEqual(["img_untracked"]);
  });
});
