...
describe("project thread resume lineage", () => {
  test("blocks resume evidence when the previous turn is not the recovered worker turn", () => {
    // Given
    ...
    const result = evaluateProjectThreadResumeEvidence({
      ...
      evidence: {
        threadId: "thread_plan_live",
        previousTurnId: "turn_unrelated_worker",
