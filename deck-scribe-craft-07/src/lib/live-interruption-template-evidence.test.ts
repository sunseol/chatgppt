...
describe("live interruption template evidence paths", () => {
  test("blocks template recovery snapshot paths", () => {
    // Given
    const matrix = completeMatrix({
      scenarios: LIVE_INTERRUPTION_SCENARIOS.map((id) =>
        id === "text_turn_shutdown"
          ? scenario(id, { recoverySnapshotPath: "recovery/text-turn-template.json" })
          : scenario(id),
      ),
    });
    ...
    expect(result.issues.map((issue) => issue.code)).toEqual(["missing_recovery_snapshot"]);
    expect(result.issues[0]?.refs).toEqual(["text_turn_shutdown"]);
  });
});
