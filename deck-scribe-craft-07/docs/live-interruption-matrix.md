...
- `duplicate_cancel_evidence_path`: cancellation recovery snapshot and cancel signal evidence reuse one JSON path.
- `cancel_signal_job_mismatch`: cancel signal evidence targets a different or missing live job id.
- `noncanonical_interruption_evidence_identity`: durable live job ids or evidence paths only become valid after trim or dot-segment normalization.
...
- `missing_interruption_report`: the matrix report path is not exactly `docs/live-interruption-matrix.md`, or that committed evidence path is missing, synthetic, or developer-local.
