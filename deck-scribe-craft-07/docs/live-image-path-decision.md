...
- Incomplete decision evidence remains blocked with `missing_billing_owner`, `missing_required_permissions`, `invalid_image_binary`, `missing_binary_artifact`, `invalid_binary_artifact_path`, `artifact_provider_mismatch`, `missing_request_model`, `artifact_model_mismatch`, or `missing_request_id`.
...
- `bun test src/lib/image-path-decision.test.ts src/lib/image-path-decision-request-model.test.ts` passes: 10 tests.
