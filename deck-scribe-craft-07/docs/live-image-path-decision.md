...
- Incomplete decision evidence remains blocked with `missing_billing_owner`, `missing_required_permissions`, `invalid_image_binary`, `missing_binary_artifact`, `invalid_binary_artifact_path`, `artifact_provider_mismatch`, `artifact_model_mismatch`, or `missing_request_id`.
...
- Excluded routes.
- Versioned project image binary artifact path, provider request id, and provider request model when a real image artifact exists.
- The production project-level `imagePathDecision` used by the Generate stage gate.

## Verification

- `bun test src/lib/image-path-decision.test.ts` passes: 9 tests.
- `bun test src/lib/image-path-decision.test.ts src/lib/image-provider-feasibility.test.ts src/lib/slide-image-provider.test.ts` passes: 17 tests.
