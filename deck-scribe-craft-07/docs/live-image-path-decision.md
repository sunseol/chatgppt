...
- A path is `locked` only when setup is ready, OpenAI organization verification is confirmed when the API-key route is used, the decision records billing owner and provider permissions, the selected provider route matches the stored artifact provider, the stored artifact request model matches the selected route model, the artifact contains PNG binary data with a PNG signature, a binary artifact path is present, and OpenAI image artifacts include a request id.
- Without one real image artifact, the decision is `blocked` with `missing_real_image_artifact`.
- Incomplete setup remains blocked with setup codes such as `requiresApiCredential` or `requiresOrganizationVerification`; `unknown` OpenAI organization verification is treated as not ready rather than production-lockable.
- Incomplete decision evidence remains blocked with `missing_billing_owner`, `missing_required_permissions`, `invalid_image_binary`, `missing_binary_artifact`, `artifact_provider_mismatch`, `artifact_model_mismatch`, or `missing_request_id`.
...
- Excluded routes.
- Binary artifact path, provider request id, and provider request model when a real image artifact exists.
- The production project-level `imagePathDecision` used by the Generate stage gate.

## Verification

- `bun test src/lib/image-path-decision.test.ts` passes: 7 tests.
- `bun test src/lib/image-path-decision.test.ts src/lib/image-provider-feasibility.test.ts src/lib/slide-image-provider.test.ts` passes: 16 tests.
