...
- Codex OAuth image batches are accepted with canonical `threadId` and `turnId` instead of OpenAI-style `requestId`; stored metadata and provenance must preserve the same Codex turn identity.
