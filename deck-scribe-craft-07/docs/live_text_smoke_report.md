...
- `bun test src/lib/live-text-smoke-gate.test.ts src/lib/live-text-smoke-prompt-lineage.test.ts` covers the full text smoke bundle contract, disconnected stage lineage rejection, stage-wrong prompt rejection, mock/fixture and missing-turn rejection, and the requirement for a completed live Codex production post-resume next turn on the same thread as the previous text-artifact turn.
