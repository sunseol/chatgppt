...
The local restart smoke evaluator rejects same-pid restart evidence, blank crash-probe output, blank post-restart thread/turn ids, a post-restart health turn captured with a different CLI version, or a restart claim that reuses the same pre-restart health thread and turn before DF-210 can count the restart path as ready.
...
| 비정상 종료 후 재시작 경로               | pass    | `kill -9` made the control socket refuse connections; `daemon start` launched new pid `5889`; post-restart health turn completed; the local gate rejects same-pid restarts, blank crash probes, blank post-restart turn ids, CLI-version mismatches, and reused pre-restart health turns.                                                                                                                                                                                                                                                                                                                                 |
