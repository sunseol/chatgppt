...
- `bun test src/lib/live-text-smoke-gate.test.ts src/lib/live-text-smoke-prompt-lineage.test.ts` covers the full text smoke bundle contract, disconnected stage lineage rejection, stage-wrong prompt rejection, mock/fixture and missing-turn rejection, and the requirement for a completed live Codex production post-resume next turn.
...
| 프로젝트 재개 후 다음 turn 실행 가능     | partial | `thread/resume` plus a second turn succeeded for the App Server health thread, the smoke bundle gate requires a completed live Codex production next turn after a produced text-artifact turn, and the DF-212 recheck resumed recovered worker thread `019edc28-bf27-7380-b7d2-65405e6c6758` from pre-restart turn `019edc28-c179-7453-a5a5-c87e29096422` to post-restart turn `019edc28-f9ec-72e1-9695-1a9a2c2ca61d`; packaged DeckForge project reopen/resume is not verified.                                                                                                                                           |
