...
- `bun test src/lib/live-text-pipeline-artifact-identity.test.ts src/lib/desktop-live-text-pipeline-workflow.test.ts` covers whitespace-padded text artifact/turn reuse rejection, the app-level desktop launcher calling three structured turns before persistence, and blocking before turn invocation when prerequisites are missing.
