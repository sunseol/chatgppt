...
- DF-240 remains blocked. Lane E has real text-stage turn ids and Lane D has
  ...
  compositor hashes.
- DF-244 is ready. The packaged dry-run app-surface smoke captures visible
  usage display evidence plus a same-run persisted Codex OAuth image billing
  confirmation JSON copied into `docs/live-evidence/...`.
...
DF-244 packaged app-surface smoke now captures real image usage payloads and
the matching persisted confirmation JSON from the same run.
...
image, billing, and queue artifacts from a real Codex image turn. The packaged
DF-244 smoke now carries the persisted packaged confirmation and regenerated
usage evidence; DF-233 still needs genuine 429/5xx retry,
