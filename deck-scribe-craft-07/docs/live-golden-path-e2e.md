...
- Observed Golden Path report, screenshot, recording, and validation bundle paths must be canonical durable values: no boundary whitespace, no developer-local absolute path, no remote URL-only path, and no `template`, `sample`, `example`, or `placeholder` marker
...
- One approved live full-slide regeneration image artifact, separate from the five initial images, that is marked as regeneration evidence and cites one of the initial live image artifacts as a canonical input artifact id
...
- `missing_validation_bundle`: final validation bundle manifest is absent or points at a boundary-whitespace-padded, synthetic mock/fixture/test/fake, template/sample/example/placeholder, developer-local, or remote URL-only path.
...
- `missing_regenerated_live_image_artifact`: no approved live full-slide regeneration image artifact is present after the initial five-image requirement is met, including marker-only regeneration artifacts or trim-only input artifact ids that do not canonically cite an initial live image as input.
