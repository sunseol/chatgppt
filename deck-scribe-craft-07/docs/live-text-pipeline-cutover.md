...
- Five-slide outputs must share one canonical `deckContextId` and one
  canonical `designSystemId`; ids that only match after trimming whitespace
  block with `deck_context_mismatch` or `design_system_mismatch`.
...
- It blocks slide refs that drift from the shared deck context or design system, including shared context ids that only become canonical after trimming.
