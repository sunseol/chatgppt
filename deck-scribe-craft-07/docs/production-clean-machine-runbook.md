...
340,252 bytes, 32 archive members, and 22 app files. It also verifies the
...
Package archive and native macOS bundle paths must be persisted non-synthetic, non-local evidence paths; `file://`, absolute developer-local, mock, fixture, test, fake, transient `tmp`/`temp`/observer paths, or dry-run archive paths do not count as DF-245 package evidence. A dry-run package can remain useful scan evidence, but it cannot satisfy production package readiness even when `productionMode` is true. The clean-machine runbook must be the canonical `docs/production-clean-machine-runbook.md`; a separate or renamed markdown file that merely ends with the same filename does not count.

Native macOS release trust evidence must include a Developer ID team signature, a canonical unpadded 10-character uppercase alphanumeric Apple TeamIdentifier, notarization, stapling, Gatekeeper acceptance, and a persisted non-synthetic, non-local `releaseTrustEvidencePath` JSON bundle whose path identifies a release-trust bundle containing the codesign, notarytool, stapler, and `spctl` assessment records. The TeamIdentifier itself must be canonical without boundary whitespace, so values such as ` TEAMID1234 ` block instead of trimming into readiness. The path itself must be canonical without boundary whitespace and must carry the `release-trust`, `codesign`, `notarytool`, `stapler`, and `spctl` markers so a generic `macos-release-trust.json` claim cannot stand in for the full assessment bundle. The persisted payload must be a `macos_release_trust` record whose package path/hash, native macOS bundle path/hash, TeamIdentifier, `notarizationStatus: "accepted"`, `stapled: true`, `gatekeeperAccepted: true`, and passed zero-exit `codesign`, `notarytool`, `stapler`, and `spctl` command records match the evaluated release. Ad-hoc signatures, missing TeamIdentifier values, placeholder values such as `not set`, boundary-whitespace-padded TeamIdentifier values, missing, non-canonical, generically named, developer-local, or structurally incomplete release-trust evidence bundles, unstapled notarization tickets, or rejected `spctl` assessments block DF-245.

The clean-machine checklist must include distinct evidence events for install, Codex login, image credential setup, project launch, and first live interview. Each step must cite its own persisted non-synthetic, non-local JSON evidence path whose path identifies that specific checklist step and is canonical without boundary whitespace; for example, `codex_login` cannot point at `install-app.json`, and one shared evidence path that names every step cannot satisfy the whole checklist. Each persisted step payload must be a `clean_machine_step` record whose `step`, `evidencePath`, `accountEvidencePath`, `macosUsername`, `homeDirectory`, `status: "passed"`, and timestamp match the checklist entry and clean account record. Repeating one step, adding an unsupported runtime step name, listing step labels without persisted evidence paths, reusing another step's evidence path, reusing one multi-step evidence path, padding a path with boundary whitespace, omitting the clean account identity, drifting to another macOS username/home directory, or attaching a payload for another step cannot inflate the completed step count or replace a missing clean-machine action.

Clean-machine validation also requires a separate persisted non-synthetic, non-local `cleanMachineAccountEvidencePath` JSON record at the canonical path `release-evidence/clean-machine/clean-macos-account.json`. The payload must be a `clean_macos_account` record with `accountType: "clean_macos_account"`, nonblank macOS username/home-directory evidence, `developerAccount: false`, and a timestamp. Generic account files that merely borrow the `clean-machine` directory or `macos-account` marker, developer-local account records, padded account paths, renamed account files, step-only evidence paths, or payloads that identify the developer account cannot prove that the checklist ran from a fresh macOS account instead of the developer user.
