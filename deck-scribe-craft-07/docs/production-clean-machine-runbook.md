...
Clean-machine validation also requires a separate persisted non-synthetic, non-local `cleanMachineAccountEvidencePath` JSON record whose path identifies both `clean-machine` and `macos-account` evidence and is canonical without boundary whitespace. Generic account files, developer-local account records, padded account paths, or step-only evidence paths cannot prove that the checklist ran from a fresh macOS account instead of the developer user.
