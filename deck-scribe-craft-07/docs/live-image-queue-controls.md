...
- `src/lib/live-image-queue-evidence.ts` rejects retry evidence that is not anchored to a recorded provider job and slide-generation prompt usage, retry event counts that do not match the final job attempt, retry events tied to a different bundle, non-transient retry failure kinds, and cancellation failures without a cancelled provider job plus preserved cancel signal.
