...
- active live jobs are marked stale when upstream approval invalidates their deck context, including jobs whose `deckContextId` still matches but whose recorded `deckContextHash` differs from the current frozen context hash or is missing while the current frozen context hash is known
...
- `src/lib/project-thread-stale-context-hash.test.ts` verifies active live jobs become stale when the context hash changes under the same `deckContextId`, treats same-context jobs with missing recorded hashes as stale when the current hash is known, and preserves id-only stale checks only when the current hash is unknown.
