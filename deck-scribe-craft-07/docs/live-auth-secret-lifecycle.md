...
| Store image API keys only through the OS keychain bridge                        | `connectImageApiKeySecret` accepts an injected `LiveSecretStore`, writes the trimmed secret through `saveSecret`, rejects non-`os_keychain` stores including `equivalent_secret_store`, rejects secret references that echo raw, URL-encoded, base64, base64url, or hex key material, rejects any returned unsupported store kind or store-kind mismatch, rejects references with the wrong service or account scope, rejects blank or boundary-whitespace-padded reference identity fields, rejects drifted created timestamp evidence, and returns only a `LiveSecretReference` plus public label. `disconnectImageApiKeySecret` rejects store-kind mismatch, wrong image credential service scope, or invalid reference identity before delete, deletes the stored key through `deleteSecret`, and returns a missing-credential state without returning the secret reference. |
...
| Logout cancels related live jobs and locks UI                                   | `cancelLiveJobsForAuthLogout` requests cancellation for active live jobs, `createLiveAuthLogoutLockState` returns `uiLocked: true` with `requiresAuth` provider statuses, and the production image generation gate blocks stale locked Codex decisions with `provider_auth_required` when the current provider status requires login. |

## Codex OAuth image route guard

The product production route is Codex OAuth image generation, not OpenAI API-key image generation. `decideImageProviderFeasibility` selects `codex` / `codexOAuth` and records `openaiApiKey` as an excluded route. `selectProviderOptionsForRuntime` now filters production provider choices to Codex-only, and `createProviderCapabilityMatrixView` turns legacy `openaiImage` / `openaiApiKey` fallback states into a Codex image capability lock instead of showing session API-key remediation. `getProductionImageProviderChoices` also excludes legacy locked `openaiImage` / `openaiApiKey` records, and `createProductionImageGenerationGate` rejects those old persisted decisions with `production_codex_oauth_required`, so a project reload cannot turn legacy API-key image evidence into a production-ready generation route or selectable provider choice.

`createProductionImageGenerationGate` also accepts the current provider status list from the app surface. When a locked `codex` / `codexOAuth` image path decision is present but the current Codex provider status is `requiresAuth`, the gate returns `provider_auth_required` instead of `ready`. `GenerateStage` passes that status into the gate so logout/disconnect lock state cannot be bypassed by reusing an older persisted image path decision.
...
`src/lib/live-auth-keychain-store.test.ts` covers the local false-ready case where an `equivalent_secret_store` adapter previously made an image credential look stored without the packaged OS keychain bridge. `src/lib/live-auth-secret-reference-identity.test.ts` covers blank or boundary-whitespace-padded secret reference identities so a store cannot return an empty `secretId` or trim-only delete target as credential evidence.
...
`src/lib/live-auth-lifecycle-disconnect-scope.test.ts` proves that disconnect
cannot delete or accept a `LiveSecretReference` whose service is outside the
DeckForge image credential scope. A wrong-service keychain reference fails with
`LiveSecretReferenceScopeError` before the keychain delete callback runs, so a
logout or cleanup path cannot count another secret as the image credential
lifecycle evidence.
