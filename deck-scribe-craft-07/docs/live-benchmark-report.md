...
- passed benchmark bundle manifests with matching evidence counts, distinct observed scenario report and `live_e2e_report.md` paths, no report, Golden Path, or screenshot path that borrows another DF-242 scenario id, distinct non-synthetic final export artifact id, at least 10 distinct observed step screenshot `.png` paths that name every Golden Path step, at least 3 distinct canonical non-synthetic source artifact ids, at least 5 distinct canonical non-synthetic initial live image artifact ids, one separate approved non-synthetic regenerated live image artifact id that is canonical, and at least 5 distinct non-synthetic live image provider turn ids
...
- `output_bundle_scenario_evidence_mismatch` - passed benchmark report, Golden Path, or screenshot paths borrow another DF-242 scenario id.
