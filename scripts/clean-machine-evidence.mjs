...
const outDir = path.resolve(
  root,
  process.env.DECKFORGE_CLEAN_MACHINE_OUT_DIR ?? `.omx/artifacts/clean-machine-${Date.now()}`,
);
const expectedDmgSha256 =
  readOption("--expected-dmg-sha256") ?? process.env.DECKFORGE_CLEAN_MACHINE_DMG_SHA256 ?? null;

const manifestPath = manifestArg ? path.resolve(root, manifestArg) : null;
const verification = await verifyCleanMachineEvidence(manifestPath, {
  expectedDmgSha256,
});
