...
import type { SlideImageArtifact, SlideImageCanvas } from "./slide-image-provider";
...
export function composeFinalSlide(input: {
  ...
  return {
    ...
    svg: renderSvg(input.background, input.layers),
...
function renderSvg(background: SlideImageArtifact, model: MvpEditableLayerModel): string {
  return [
    ...
    )}" x="0" y="0" width="${background.canvas.width}" height="${background.canvas.height}" preserveAspectRatio="xMidYMid slice" />`,
