  ...
  head: () => ({
    meta: [
      { title: "DeckForge — 검증 가능한 AI 슬라이드 제작 시스템" },
      {
        name: "description",
        content:
          "인터뷰 · 조사 · 기획 · 디자인 · 레이아웃 · 생성 · 검증 · 편집까지 승인 기반으로 강제 수행하는 슬라이드 제작 하네스의 웹 MVP 프로토타입.",
      },
      { property: "og:title", content: "DeckForge — 검증 가능한 AI 슬라이드 제작 시스템" },
      {
        property: "og:description",
        content: "승인 기반 강제 워크플로우로 일관된 슬라이드를 만드는 웹 프로토타입.",
      },
    ],
  }),
  component: Home,
    ...
    <div className="min-h-screen">
            ...
            <DialogContent className="max-h-[calc(100vh-4rem)] max-w-2xl overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription className="sr-only">
                  새 프로젝트의 기본 정보와 provider 기능 상태를 설정합니다.
                </DialogDescription>
              </DialogHeader>
        ...
        <section className="mb-16 grid grid-cols-12 gap-8">
          <div className="col-span-12 md:col-span-7">
            <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-accent">
              v0.1 · Workflow Prototype
            </div>
            <h1 className="mt-3 font-serif text-5xl leading-[1.05] tracking-tight md:text-6xl">
              생성이 아니라
              <br />
              <span className="text-accent">검증</span>으로 완성되는
              <br />
              슬라이드 제작.
            </h1>
            <p className="mt-6 max-w-xl text-base text-ink-soft">
              DeckForge는 인터뷰 → 조사 → 기획 → 디자인 시스템 → HTML 레이아웃 → 이미지 생성 → 검증
              → 편집화 → 보고를
              <strong className="text-foreground">
                {" "}
                사용자 승인 없이 다음 단계로 넘어가지 않는
              </strong>{" "}
              강제 워크플로우로 수행하는 데스크탑 PPT 제작 시스템의 웹 프로토타입입니다.
            </p>
          </div>
          <div className="col-span-12 md:col-span-5">
            <div className="border-l border-border pl-6">
              ...
              <ul className="mt-3 space-y-3 text-sm">
                <li>
                  <strong className="font-medium">Approval Gate</strong> · 승인 버튼이 유일한 상태
                  전이 경로
                </li>
                <li>
                  <strong className="font-medium">Frozen Context</strong> · 승인된 산출물의 해시를
                  다음 단계가 참조
                </li>
                <li>
                  <strong className="font-medium">Downstream Invalidation</strong> · 상위 수정 시
                  이후 자동 무효화
                </li>
                <li>
                  <strong className="font-medium">Editable Output</strong> · PNG 한 장이 아닌 레이어
                  모델로 종결
                </li>
              </ul>
            </div>
        ...
        <section>
          ...
          {!hydrated ? (
            <div className="border border-dashed border-border bg-paper p-12 text-center">
              <div className="text-sm text-muted-foreground">프로젝트 목록 불러오는 중</div>
            </div>
          ) : projects.length === 0 ? (
            <div className="border border-dashed border-border bg-paper p-12 text-center">
              <div className="text-sm text-muted-foreground">
                아직 프로젝트가 없습니다. 샘플 프롬프트로 빠르게 시작해보세요.
              </div>
              <Button
            ...
            <ul className="divide-y divide-border border border-border bg-paper">
              {projects.map((p) => {
                ...
                return (
                  <li
                    key={p.id}
                    className="group grid grid-cols-[1fr_140px_140px_120px_60px] items-center gap-4 px-5 py-4"
                  >
                    <div className="min-w-0">
                      ...
                      <LocalProjectDataControls
                        ...
                        onExport={() => downloadLocalProjectFolder(p)}
                        onDelete={() => {
                          if (confirm("이 로컬 프로젝트를 삭제할까요?")) deleteProject(p.id);
                        }}
                      />
                    </div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {invalidated > 0 ? (
                        <span className="text-warning">{invalidated} invalidated</span>
                      ) : (
                        `${p.aspectRatio} · ${p.slideCount}장`
                      )}
                    </div>
                    <div className="text-xs text-muted-foreground">
                    ...
                    <div className="flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <Button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          if (confirm("이 프로젝트를 삭제할까요?")) deleteProject(p.id);
                        }}
                      >
      ...
      <footer className="border-t border-border bg-paper py-8">
        <div className="mx-auto max-w-6xl px-8 text-xs text-muted-foreground">
          DeckForge Web Prototype · 모든 AI 출력은 데모용으로 모킹되어 있으며, 워크플로우 강제성과
          상태 전이 검증이 목적입니다.
        </div>
      </footer>
    </div>
